...
class EvaluasiRKPDPerubahanModel extends ReportModel
{   
    public function __construct($dataReport,$print=true)
    {
        parent::__construct($dataReport); 
        $this->spreadsheet->getProperties()->setTitle("Laporan Evaluasi RKPD Tahun ".$this->dataReport['tahun']);
        $this->spreadsheet->getProperties()->setSubject("Laporan Evaluasi RKPD Tahun ".$this->dataReport['tahun']); 
        if ($print)
        {
            $this->print();             
        }        
    ...
    private function print()  
    {
        ...
        $sheet->setTitle ('LAPORAN EVALUASI RKPD PERUBAHAN');
        ...
        $sheet->setCellValue("A$row",'Evaluasi Terhadap Hasil RKPD');         
        $row+=1;                
        $sheet->mergeCells("A$row:AA$row");		
        ...
        $sheet->setCellValue("G$row",'Urusan/Bidang Pemerintahan Daerah Dan Program/Kegiatan');
        ...
        $sheet->setCellValue("I$row",'Target RPJMD Tahun '.$tahun_akhir_rpjmd);
        $sheet->mergeCells("K$row:L$row_akhir");
        $sheet->setCellValue("K$row",'Realisasi Capaian Kinerja RPJMD s/d RKPD Tahun Lalu ('.($tahun-1).')');                
        $sheet->mergeCells("M$row:N$row_akhir");
        $sheet->setCellValue("M$row",'Target Kinerja dan Anggaran RKPD Tahun Berjalan (N-1) yang Dievaluasi');                
        ...
        $sheet->setCellValue("W$row","Realisasi Kinerja dan Anggaran RPJMD s/d Tahun $tahun");                
        $sheet->mergeCells("Y$row:Z$row_akhir");
        $sheet->setCellValue("Y$row","Tingkat Capaian Kinerja dan Realisasi Anggaran RPJMD s/d Tahun $tahun");                
        ...
        foreach ($daftar_program as $data_program)
        {
            ...
            $daftar_kegiatan = \DB::table('trRKA')
                                    ->select(\DB::raw('"RKAID","kode_kegiatan","KgtNm",capaian_program2,tk_capaian2,keluaran2,hasil1,tk_keluaran2,tk_hasil2,"PaguDana2","lokasi_kegiatan2"'))
            ...
            if(isset($daftar_kegiatan[0]))
            {
                ...
                foreach ($daftar_kegiatan as $n)
                {
                    ...
                    $indikator_kinerja=empty(trim($n->hasil1))?'':$n->hasil1.' ('.$n->tk_hasil2.')';
