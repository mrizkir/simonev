...
use App\Models\Report\FormAPerubahanModel;
...
class FormAPerubahanController extends Controller 
{
    ...
    {             
        ...
        {  
            ...
            foreach ($tingkat_1 as $k1=>$v1)
            {
                foreach ($tingkat_5 as $k5=>$v5)
                {
                    ...
                    if ($rek1 == $k1) 
                    {
                        ...
                        $totalPaguDana_Rek1=\App\Models\Report\FormAPerubahanModel::calculateEachLevel($rka,$k1,'Kd_Rek1');   
                        $totalPaguDana=$totalPaguDana_Rek1['totalpagu'];                                             
                        $data[]=[
                            'FormAPerubahanDetailID'=>uniqid('uid'),  
                            'tingkat'=>1,                          
                            'kode'=>$k1,                            
                            'nama_uraian'=>$v1,
                        ...
                        foreach ($tingkat_2 as $k2=>$v2) 
                        {
                            ...
                            {
                                $totalPaguDana_Rek2=\App\Models\Report\FormAPerubahanModel::calculateEachLevel($rka,$a,'Kd_Rek2');                                
                                $no_=explode ('.',$a);                                                   
                                $data[]=[
                                    'FormAPerubahanDetailID'=>uniqid('uid'),   
                                    'tingkat'=>2,         
                                ...
                                foreach ($tingkat_3 as $k3=>$v3) 
                                {
                                    ...
                                    if ($a==$rek3) 
                                    {
                                        $totalPaguDana_Rek3=\App\Models\Report\FormAPerubahanModel::calculateEachLevel($rka,$k3,'Kd_Rek3');                                        
                                        $no_=explode (".",$k3);
                                        ...
                                        foreach ($tingkat_4 as $k4=>$v4) 
                                        {
                                            if (preg_match("/^$k3/", $k4)) 
                                            {
                                                ...
                                                foreach ($tingkat_5 as $k5=>$v5) 
                                                {
                                                    ...
                                                    {      
                                                        $totalUraian+=1;
                                                        $totalPaguDana_Rek5=\App\Models\Report\FormAPerubahanModel::calculateEachLevel($rka,$k5,'Kd_Rek5');													                                                        
            ...
            $total_data=[
                            'totalPaguDana'=>$totalPaguDana,                            
                            'totalPersenBobot'=>number_format($totalPersenBobotSatuKegiatan,2),                            
    ...
    public function printtoexcel (Request $request)
    {
        ...
        if (\DB::table('trRKARinc')->where('RKAID',$RKAID)->count()>0)
        {
            ...
            $report= new \App\Models\Report\FormAPerubahanModel ($data_report);
