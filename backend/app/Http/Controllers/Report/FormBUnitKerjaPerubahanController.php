...
class FormBUnitKerjaPerubahanController extends Controller 
{
    ...
    {             
        $this->hasPermissionTo('FORM B PERUBAHAN_BROWSE');
        ...
        foreach ($daftar_program as $data_program)
        {
            ...
            if(isset($daftar_kegiatan[0]))
            {
                ...
                $persen_bobot_program=Helper::formatPersen($totalpagueachprogram,$totalPaguUnit);
                ...
                $persen_realisasi_fisik_program=Helper::formatPecahan($data_realisasi_program[0]->fisik2,$jumlahuraian_program);
                ...
                $persen_realisasi_keuangan_program=Helper::formatPersen($data_realisasi_program[0]->realisasi2,$totalpagueachprogram);  
                ...
                $sisa_anggaran_program=$totalpagueachprogram-$data_realisasi_program[0]->realisasi2;
                $persen_sisa_anggaran_program=Helper::formatPersen($sisa_anggaran_program,$totalpagueachprogram);  

                $data[]=[
                    'FormBPerubahanID'=>uniqid('uid'),
                    'RKAID'=>null,
                    'kode'=>$kode_program,
                    'nama'=>$data_program->PrgNm,
                    'pagu_dana2'=>$totalpagueachprogram,
                    'bobot2'=>$persen_bobot_program,
                    'fisik_target2'=>$persen_target_fisik_program,
                    'fisik_realisasi2'=>$persen_realisasi_fisik_program,
                    'fisik_ttb2'=>$persen_tertimbang_fisik_program,
                    'keuangan_target2'=>$data_target_program[0]->totaltarget,
                    'keuangan_target_persen_2'=>$persen_target_keuangan_program,
                    'keuangan_realisasi2'=>$data_realisasi_program[0]->realisasi2,
                    'keuangan_realisasi_persen_2'=>$persen_realisasi_keuangan_program,
                    'keuangan_ttb2'=>$persen_tertimbang_keuangan_program,
                    'lokasi'=>'-',
    ...
    public function printtoexcel (Request $request)
    {
        ...
        $unitkerja = SubOrganisasiModel::find($SOrgID);
        if (\DB::table('trRKA')->where('kode_suborganisasi',$unitkerja->kode_suborganisasi)->where('EntryLvl',2)->where('TA',$tahun)->count()>0)
        {
            $data_report=[
                            'kode_suborganisasi'=>$unitkerja->kode_suborganisasi,
            ...
            $report= new \App\Models\Report\FormBUnitKerjaPerubahanModel ($data_report);
