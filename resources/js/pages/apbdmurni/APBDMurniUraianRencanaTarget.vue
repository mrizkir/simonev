                            ...
                            <table class="table table-striped table-hover mb-2">
                                ...
                                <tbody>  
                                    <tr v-for="item in daftar_targetfisik" v-bind:key="item.RKARincID">
                                        ...
                                        <td>
                                            {{ item.nama_uraian }}<br>
                                            [<em>{{ item.pagu_uraian1|formatUang }}</em>]
                                        </td>
                                        <td>{{ item.fisik_1 }}</td>
                                        <td>{{ item.fisik_2 }}</td>
                                        <td>{{ item.fisik_3 }}</td>
...
export default {
    ...
    {	
        ...
        fetchPaguUraian ()
        {
            ...
            .then(response => {                                                                
                this.form.pagu_uraian=parseFloat(response.data.pagu_uraian1);
            })
            .catch(response => {
                this.api_message = response;
            });
        ...
        { 
            ...
            {                
                ...
                case 'edit' :
                    ...
                    this.form.pagu_uraian=item.pagu_uraian1;                      
                    axios.get('/api/v1/apbdmurni/'+item.RKARincID+'/edit3',{
                        headers:{
                            'Authorization': window.laravel.api_token,
                        }
                    })
		...
		{	
            ...
            if(this.$v.$invalid == false)
            { 
                axios.post('/api/v1/apbdmurni/update3',{                   
                    'RKARincID':this.form.RKARincID,
                    'RKAID':this.detailkegiatan.RKAID,
                    'bulan_fisik':this.form.targetfisik,                   
                    'bulan_anggaran':this.form.targetanggarankas,                   
