           ...
           <template v-slot:icon>
            ...
            <template v-slot:desc>
                <v-alert                                        
                    ...
                    type="info"
                    >
                    Laporan Evaluasi RKPD (Rencana Kerja Pemerintah Daerah) Perubahan
                </v-alert>
            </template>
        </ModuleHeader>
        <v-container fluid>    
             ...
             <v-row class="mb-4" no-gutters>
                <v-col cols="12">
                    <v-data-table
                        ...
                        :items="datatable"
                        :search="search"
                        item-key="EvaluasiRKPDPerubahanID"                        
                        dense                        
                        :single-expand="true"
                        class="elevation-1"
                        ...
                        <template v-slot:body="{ items }">
                            <tbody>
                                <tr v-for="(item) in items" v-bind:key="item.FormBPerubahanID" v-bind:class="{'indigo lighten-4 font-weight-bold':isProgram(item)}">
                                    ...
                                    <td class="text-right">{{item.fisik_target2|formatPersen}}</td>                                                                        
...
export default {
    ...
    {
        this.fetchOPD(); 
        var OrgID_Selected=this.$store.getters['uiadmin/AtributeValueOfPage']('evaluasirkpdperubahan','OrgID_Selected');          
    ...
    data ()
    {
        return {
            ...
            headers: [                        
                ...
                { text: 'TARGET ANGGARAN', value: 'pagu_dana2',align:'end',width:100,sortable:false},                                                      
