        ...
        <ModuleHeader>
            ...
            <template v-slot:desc>
                <v-alert
                    ...
                    type="info"
                    >
                     User SIMONEV ditampilkan berdasarkan role opd.
                </v-alert>
            </template>
        </ModuleHeader>
        <v-container fluid>
                                ...
                                <v-dialog v-model="dialog" max-width="500px" persistent>
                                    <v-form ref="frmdata" v-model="form_valid" lazy-validation>
                                        <v-card>
                                            ...
                                            <v-card-subtitle>
                                                Bila OPD / SKPD, tidak dipilih artinya user ini tidak dapat mengakses data opd.
                                            </v-card-subtitle>
                                            <v-card-text>
                                                <v-text-field
...
export default {
    ...
    data: () => ({        
        ...
        rule_user_payload:[
            value => value.length>0||"Mohon untuk di pilih OPD / SKPD dari User ini !!!",
        ], 
    }),
    methods: {
        ...
        save () {
            if (this.$refs.frmdata.validate())
            {
                ...
                if (this.editedIndex > -1)
                {
                    this.$ajax.post('/setting/usersopd/'+this.editedItem.id,
                        {
                            ...
                            payload:JSON.stringify(Object.assign({},this.editedItem.payload)),                            
                        },
                        {
                            headers:{
                                Authorization:this.TOKEN
                    ...
                    this.$ajax.post('/setting/usersopd/store',
                        {
                            name:this.editedItem.name,
                            email:this.editedItem.email,                            
                            username:this.editedItem.username,
    ...
    computed: {
        formTitle () {
            return this.editedIndex === -1 ? 'TAMBAH USER OPD' : 'EDIT USER OPD'
        },
        ...mapGetters('auth',{
