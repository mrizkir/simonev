...
{   
    ...
    private function print()  
    {
        ...
        $rka=$this->getDataRKA($RKAID,$no_bulan,$this->dataReport['entryLvl']);
        ...
        $sheet->setCellValue("G$row",': ' .$datakegiatan['lokasi_kegiatan1']);                
        $row+=1;  
        $sheet->mergeCells("B$row:F$row");
        ...
        $sheet->mergeCells("G$row:K$row");
        $sheet->setCellValue("G$row",$datakegiatan['capaian_program1']);
        $sheet->mergeCells("L$row:N$row");
        $sheet->setCellValue("L$row",$datakegiatan['tk_capaian1'] . "%");    
        $row+=1;                
        $sheet->mergeCells("B$row:F$row");
        $sheet->setCellValue("B$row",'MASUKAN');
        $sheet->mergeCells("G$row:K$row");
        $sheet->setCellValue("G$row",$datakegiatan['masukan1']);
        $sheet->mergeCells("L$row:N$row");
        $sheet->setCellValue("L$row",\Helper::formatUang($datakegiatan['PaguDana1']));
        $row+=1;                
        $sheet->mergeCells("B$row:F$row");
        $sheet->setCellValue("B$row",'KELUARAN');
        $sheet->mergeCells("G$row:K$row");
        $sheet->setCellValue("G$row",$datakegiatan['keluaran1']);
        $sheet->mergeCells("L$row:N$row");
        $sheet->setCellValue("L$row",$datakegiatan['tk_keluaran1']);
        $row+=1;                                
        $sheet->mergeCells("B$row:F$row");
        $sheet->setCellValue("B$row",'HASIL');
        $sheet->mergeCells("G$row:K$row");
        $sheet->setCellValue("G$row",$datakegiatan['hasil1']);
        $sheet->mergeCells("L$row:N$row");
        $sheet->setCellValue("L$row",$datakegiatan['tk_hasil1']);
        $row+=1;                
        $sheet->mergeCells("B$row:F$row");
        $sheet->setCellValue("B$row",'KELOMPOK SASARAN KEGIATAN');                                
        $sheet->mergeCells("G$row:J$row");
        $sheet->setCellValue("G$row",$datakegiatan['ksk1']);
        $row+=1;                                
                ...
                foreach ($tingkat_5 as $k5=>$v5) {
                    ...
                    if ($rek1 == $k1) {
                        ...
                        foreach ($tingkat_2 as $k2=>$v2) 
                        {
                            ...
                            {
                                $totalPaguDana_Rek2=$this->calculateEachLevel($rka,$a,'kode_rek_2'); 
                                $no_=explode ('.',$a);
                                ...
                                foreach ($tingkat_3 as $k3=>$v3) 
                                {
                                    ...
                                    if ($a==$rek3) 
                                    {
                                        $totalPaguDana_Rek3=$this->calculateEachLevel($rka,$k3,'kode_rek_3');                                        
                                        $no_=explode (".",$k3);
                                        ...
                                        foreach ($tingkat_4 as $k4=>$v4) 
                                        {
                                            if (preg_match("/^$k3/", $k4)) 
                                            {
                                                ...
                                                foreach ($tingkat_5 as $k5=>$v5) 
                                                {
                                                    ...
                                                    {      
                                                        $totalUraian+=1;
                                                        $totalPaguDana_Rek5=$this->calculateEachLevel($rka,$k5,'kode_rek_5');													
        ...
        $total_persen_target=number_format(($totalTargetSatuKegiatan/$totalPaguDana)*100,2);
        $sheet->setCellValue("Q$row",$total_persen_target);
        $sheet->setCellValueExplicit("R$row",$rp_total_realisasi,DataType::TYPE_STRING);                
        $total_persen_rata2_realisasi=number_format(($totalRealisasiSatuKegiatan/$totalPaguDana)*100,2);
