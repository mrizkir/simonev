						...
						<v-row dense>
								<v-col xs="12" sm="6" md="3">
										<v-card color="#385F73" dark>
												...
												<v-card-subtitle>
														Total Pagu APBD Murni TA {{$store.getters['uifront/getTahunAnggaran']}}
												</v-card-subtitle>
												<v-card-text>
														{{statistik1.PaguDana1|formatUang}}
												</v-card-text>
												<v-card-actions>
														<v-progress-linear v-model="statistik1.PersenRealisasiKeuangan1" color="success" background-color="error"></v-progress-linear>
												</v-card-actions>
										</v-card>
								</v-col>
								...
								<v-col xs="12" sm="6" md="3">
										<v-card color="#385F73" dark>
												...
												<v-card-text>
														Prog.: {{statistik1.JumlahProgram1}} / Keg.: {{statistik1.JumlahKegiatan1}}
												</v-card-text>
												<v-card-actions>
														<v-progress-linear v-model="statistik1.PersenRealisasiKeuangan1" color="success" background-color="error"></v-progress-linear>
												</v-card-actions>
								...
								<v-col xs="12" sm="6" md="3">
										<v-card color="#385F73" dark>
												...
												<v-card-text>
														{{statistik1.RealisasiKeuangan1|formatUang}} ({{statistik1.PersenRealisasiKeuangan1}}%)
												</v-card-text>
												<v-card-actions>
														<v-progress-linear v-model="statistik1.PersenRealisasiKeuangan1" color="success" background-color="error"></v-progress-linear>
												</v-card-actions>
								...
								<v-col xs="12" sm="6" md="3">
										<v-card color="#385F73" dark>
												...
												<v-card-text>
														{{statistik1.RealisasiFisik1}} %
												</v-card-text>
												<v-card-actions>
														<v-progress-linear v-model="statistik1.RealisasiFisik1" color="success" background-color="error"></v-progress-linear>
												</v-card-actions>
										</v-card>
								</v-col>
								<v-responsive width="100%" v-if="$vuetify.breakpoint.xsOnly"/>
...
export default {
		...
		methods: {
				async initialize()
				{
						await this.$ajax.post('/belanjamurni',
						{
				...
				{
						 await this.$ajax.post('/belanjamurni/reloadstatistik1',
						{
								ta:this.tahun_anggaran,                                
						},
						{
				...
				{
						 await this.$ajax.post('/belanjamurni/reloadstatistik2',
						{
								ta:this.tahun_anggaran,                                
						},
						{
